...
namespace CarService.API.Controllers
{
    ...
    public class OrdersController : ControllerBase
    {
        ...
        // ===== Лабораторная 3: Расчёт итоговой стоимости заказа =====

        /// <summary>
        /// Возвращает итоговую стоимость заказа: сумма услуг + сумма запчастей × количество.
        /// Для запчастей используется PriceAtOrder (цена на момент добавления в заказ).
        /// </summary>
        [HttpGet("{id}/total")]
        public async Task<IActionResult> GetTotal(int id)
        {
            ...
            var parts = await _db.OrderParts
                ...
                {
                    op.Part!.Name,
                    UnitPrice  = op.PriceAtOrder,
                    op.Quantity,
                    TotalPrice = op.PriceAtOrder * op.Quantity
                })
                .ToListAsync();
